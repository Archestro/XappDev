...
class UserService {
  Future<User> getUserdata(int? id,
      {isMe = false, isProfilePage = false, String? username}) async {
    var res = await http.get(Uri.http(baseUrl, '/user/me'), headers: {
      'id': id.toString(),
      'isme': isMe.toString(),
  ...
  Future<MyUser> getMyUserdata(int? id,
      {isMe = false,
      isProfilePage = false,
      String? username,
    ...
    var json = await jsonDecode(res.body);
    return MyUser.fromDoc(json,
        isMe: isMe, isProfilePage: isProfilePage, pos: pos);
  }
  ...
  Future<http.Response> attemptSignUp(
    ...
    var res = await http.post(Uri.http(baseUrl, '/user/register'), body: {
      "email": email,
      "password": password,
      "firstName": firstName,
      "lastName": lastName,
  ...
  Future<http.Response> attemptLogIn(String email, String password) async {
    var res = await http.post(Uri.https(baseUrl, '/user/login'),
        body: {"email": email, "password": password});
    return res;
  }

  Future removeFriend(User userdata) async {
    var res = await http.delete(Uri.http(baseUrl, '/removeFriend'), headers: {
  ...
  Future blockUser(User userdata) async {
    var res = await http.post(Uri.http(baseUrl, '/blockUser'), body: {
      "myId": user.id.toString(),
  ...
  Future<int> unBlockUser(int altId) async {
    var res = await http.delete(Uri.http(baseUrl, '/unblockUser'),
        headers: {"myId": user.id.toString(), "altId": altId.toString()});

    return res.statusCode;
  }

  Future getBlockedUser(int id, {int chunkSet = 0}) async {
    var res = await http.get(Uri.http(baseUrl, '/getBlockedUsers'),
        headers: {"id": id.toString(), 'chunkset': chunkSet.toString()});

    List<User> usersList = (jsonDecode(res.body) as List)
        .map((data) => User.fromDoc(data))
        .toList();
