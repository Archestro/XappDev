...
class MapScreen extends StatefulWidget {
  ...
  /// remove - replace with user.currentLocation
  final LatLng? newCords;
  final bool isLaunch;
  const MapScreen(
      {Key? key, this.currentPosition, this.newCords, this.isLaunch = false})
      : super(key: key);
...
class _MapScreenState extends State<MapScreen> {
  ...
  Future<Position?> getCurrentPosition() async =>
      currentPosition = await GeoLocationService.getLocationPermission();
  ...
  showOverlayCallback(
      {bool value = false, post, double? latitude, double? longitude}) {
    if (value == true) {
  ...
  void initState() {
    ...
    if (widget.currentPosition != null) {
      ...
      if (widget.newCords != null) {
        ...
        mapMoveEventListener(
            MapPosition(center: widget.newCords, zoom: 12, bounds: bounds));
      }
    }
    super.initState();
    ...
    timer = Timer.periodic(
        ...
        (Timer t) async => user.currentPosition =
            await GeoLocationService.getLocationPermission(update: true) ??
                user.currentPosition);
  }

  Future<List<Marker>> mapMoveEventListener(MapPosition pos,
      {bool isStatic = false, LatLngBounds? bounds}) async {
    List<Marker> marks = [];
      ...
      body: OverlayClass(
        ...
        overlay: _showOverlay
            ? OverlayScreen(
                ...
                isMe: _isMeOverlay,
                latitude: latitudeOverlay,
                longitude: longitudeOverlay,
                zoom: zoom,
                place: place)
            : const SizedBox(),
        child: Stack(
          ...
          child: Padding(
            ...
            child: AnimatedOpacity(
              ...
              child: GestureDetector(
                onTap: () {
                  mapController.move(
                      LatLng(user.currentPosition.latitude,
    ...
    return FlutterMap(
      ...
      options: MapOptions(
        ...
        onMapReady: () {
          ...
          final LatLngBounds startBounds = LatLngBounds(
              LatLng(widget.currentPosition!.latitude - 0.05,
                  widget.currentPosition!.longitude - 0.03),
              LatLng(widget.currentPosition!.latitude + 0.05,
                  widget.currentPosition!.longitude + 0.03));
          mapMoveEventListener(MapPosition(),
              isStatic: true, bounds: startBounds);
        },
      ),
      children: [
        tileLayerOptions,
  ...
  Widget mapButtonContainer(Icon icon, String route,
    ...
    return Padding(
      ...
      child: Container(
        ...
        child: IconButton(
          ...
          onPressed: () {
            if (route == 'search')
              NavigationService.fadeUpRoute(context, const SearchScreen());
            if (route == 'upload')
              NavigationService.fadeUpRoute(
                  context, const CameraCaptureScreen());
            // if(route == 'message') NavigationService.push(context, const SocialDashboard());
            if (route == 'profile')
              NavigationService.pushLeft(
                  context,
                  ProfilePage(
                    userId: user.id,
                    isLeading: false,
